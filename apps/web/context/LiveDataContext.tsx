  ...
  useEffect(() => {
    ...
    const projectEvents = (events: EventEnvelope[]): void => {
      ...
      const nextIncidents: Incident[] = [...grouped.entries()].flatMap(([id, callEvents]) => {
        ...
        const ended = callEvents.some((event) => event.event === 'call.ended');
        return [{ id, type: String(item.incident_type ?? 'Unknown'), location: String(item.location ?? 'Not confirmed'), time: new Date(incidentEvent.occurred_at).toLocaleTimeString(), status: ended ? 'Resolved' : 'Active', severity: String(item.severity ?? 'unknown').toUpperCase(), units: [], latitude: Number(item.latitude) || undefined, longitude: Number(item.longitude) || undefined, victims: Number(item.victims) || undefined, hazards: Array.isArray(item.hazards) ? item.hazards.map(String) : [], summary: String(item.summary ?? item.reply ?? ''), confidence: Number(item.ai_confidence ?? item.confidence) || undefined, transcript: callEvents.filter((event) => event.event === 'transcript.updated').map((event) => { const transcriptItem = payload(event); return { time: new Date(event.occurred_at).toLocaleTimeString(), speaker: transcriptItem.speaker === 'assistant' ? 'COPILOT_SYS' as const : 'TARGET_CALLER' as const, text: String(transcriptItem.message ?? '') }; }) }];
      });
